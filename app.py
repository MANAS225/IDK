...
# Store alarm state globally for acknowledgement simulation
alarm_state = {
    "unacknowledged": 0,
    "acknowledged": 0,
    "suppressed": 0,
...
def fetch_real_machines(auth_data):
    ...
    try:
        response = requests.get("http://localhost:8000/api/data/machines", headers=headers, timeout=2)
...
def fetch_real_alerts(auth_data):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    try:
        response = requests.get("http://localhost:8000/api/data/alerts", headers=headers, timeout=2)
        if response.status_code == 200:
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .ai-card:hover {
      box-shadow: 0 8px 24px rgba(38, 198, 218, 0.15) !important;
      transform: translateY(-2px);
    }
    ...
    .login-input    {
      ...
      color: var(--text-primary); font-size: 14px; padding: 11px 14px;
    ...
    .login-btn {
      width: 100%; background: #00E676; border: none; border-radius: 4px;
      color: #09090b; font-size: 12px; font-weight: 700; padding: 13px;
      cursor: pointer; letter-spacing: 0.1em; text-transform: uppercase;
      transition: all 0.2s ease; margin-top: 8px; font-family: 'Inter', sans-serif;
    }
    ...
    .login-error {
      ...
      border-radius: 4px; color: #FFCDD2; font-size: 12px;
    ...
    .login-success {
      ...
      border-radius: 4px; color: #A5D6A7; font-size: 12px;
      padding: 10px 14px; margin-bottom: 14px; display: none;
    }
    ...
    .role-btn:hover { border-color: #00E676 !important; }
    .role-btn.role-active { border-color: #00E676 !important; }
...
login_page = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.H2("Select role & enter name", style={"color": "#FFFFFF", "fontSize": "24px", "fontWeight": "600", "marginBottom": "32px"}),

            html.Div(id="login-error-msg",   className="login-error"),
            html.Div(id="login-success-msg", className="login-success"),

            html.Div([
                html.Label("OPERATOR NAME", style={"color": "#8B949E", "fontSize": "10px", "fontWeight": "700", "letterSpacing": "0.1em", "marginBottom": "8px", "display": "block"}),
                dcc.Input(
                    id="login-email", type="text", value="admin@hmi.com",
            ...
            html.Div([
                ...
                html.Div([
                   ...
                   style={"display": "flex", "alignItems": "center", "backgroundColor": "#09090b",
                          "border": "1px solid #27272a", "borderRadius": "4px",
                          "padding": "12px 16px", "marginBottom": "32px", "cursor": "pointer"}),
            ]),

            dcc.Input(id="login-password", type="hidden", value="demo-mode"),
            ...
            html.Div("NO PASSWORD REQUIRED — DEMO MODE", style={
...
app.layout = html.Div([
    ...
    dcc.Store(id="auth-store", data={"logged_in": False, "user": None, "role": "operator"}),
...
# ── GLOBAL DATA UPDATE (single source of truth — no duplicates) ──────────────
@app.callback(
...
def select_role(op_clicks, eng_clicks, mgr_clicks, current_role):
    ...
    role_map = {
        "role-btn-operator": "operator",
        "role-btn-engineer": "engineer",
        "role-btn-manager":  "manager",
    }
    selected = role_map.get(trigger_id, current_role)

    base = {"display": "flex", "alignItems": "center", "backgroundColor": "#09090b",
            "borderRadius": "4px", "padding": "12px 16px", "cursor": "pointer"}
    active_border   = {**base, "border": "1px solid #00E676", "marginBottom": "8px"}
    inactive_border = {**base, "border": "1px solid #27272a", "marginBottom": "8px"}
    active_border_e = {**base, "border": "1px solid #26C6DA", "marginBottom": "8px"}
    active_border_m = {**base, "border": "1px solid #FFB300", "marginBottom": "32px"}
    inactive_border_last = {**base, "border": "1px solid #27272a", "marginBottom": "32px"}

    ind_active_op  = {"width": "6px", "height": "6px", "backgroundColor": "#00E676"}
    ind_active_eng = {"width": "6px", "height": "6px", "backgroundColor": "#26C6DA"}
    ind_active_mgr = {"width": "6px", "height": "6px", "backgroundColor": "#FFB300"}
    ind_inactive   = {"width": "6px", "height": "6px", "backgroundColor": "#27272a"}

    if selected == "operator":
        return selected, active_border, {**inactive_border}, inactive_border_last, ind_active_op, ind_inactive, ind_inactive
    elif selected == "engineer":
        return selected, inactive_border, active_border_e, inactive_border_last, ind_inactive, ind_active_eng, ind_inactive
    else:
        return selected, inactive_border, inactive_border, active_border_m, ind_inactive, ind_inactive, ind_active_mgr
...
def route_app(pathname, auth_data):
    ...
    if pathname == "/signup":
        return signup_page, auth_data
    if pathname == "/logout":
        # Clear auth-store on logout
        return login_page, {"logged_in": False, "user": None, "role": "operator"}
    if not is_logged_in and pathname not in ["/login", "/logout"]:
        return login_page, auth_data

    if role == "engineer":
        sidebar = engineer_sidebar
    elif role == "manager":
        sidebar = manager_sidebar
    else:
        sidebar = operator_sidebar

    return html.Div([sidebar, html.Div(id="page-content", style=CONTENT_STYLE)]), auth_data
...
def handle_login(n_clicks, email, password, auth_data, selected_role):
    ...
    if not email or not password:
        return auth_data, "⚠ Please enter your name.", "login-error visible", "", "login-success", "/login"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            success_msg = "✓ Access Granted! Redirecting..."
            # Only verify sensor ingest for real (non-demo) tokens
            if token and token != "demo":
                try:
                    ingest_res = requests.post(
                        "http://localhost:8000/api/sensor/ingest",
                        json={"temperature": 100, "pressure": 90, "rpm": 1500, "source": "login-test"},
                        timeout=2
                    )
                    if ingest_res.status_code != 200:
                        success_msg = "⚠ Backend connected but sensor ingest failed."
                except Exception:
                    success_msg = "⚠ Backend connected but sensor ingest failed."

            return new_auth, "", "login-error", success_msg, "login-success visible", redirect_path
        else:
        ...
        return new_auth, "", "login-error", "✓ Demo Access Granted! Redirecting...", "login-success visible", redirect_path
...
def handle_alarm_actions(ack_clicks, supp_clicks, row_ack, row_supp, store, live_data):
    ...
    if "alarm-ack-all-btn" in prop_id:
        ...
        new_val = max(0, store.get("total", total) - 1)
        return {"unack": new_val, "ack": store.get("ack", 0), "supp": store.get("supp", 0) + 1, "total": new_val}
    elif '"type":"alarm-row-ack"' in prop_id:
        return {"unack": max(0, store.get("unack", total) - 1), "ack": store.get("ack", 0) + 1, "supp": store.get("supp", 0), "total": store.get("total", total)}
    elif '"type":"alarm-row-supp"' in prop_id:
        return {"unack": max(0, store.get("unack", total) - 1), "ack": store.get("ack", 0), "supp": store.get("supp", 0) + 1, "total": store.get("total", total)}
    return store
...
def generate_report(n_clicks, rtype, store, live_data):
    ...
    # Build a simple CSV content for download
    history_list = list(sensor_history)[-20:]  # last 20 readings
    ...
    <body>
        <h1>NEXUS IQ Plant - {label}</h1>
        <p><strong>Generated:</strong> {now_full}</p>
        <p><strong>Role:</strong> Manager</p>
        
        <h2>KPI Scorecard</h2>
        <table>
            <tr><th>KPI</th><th>Current Value</th><th>Target</th></tr>
            {kpi_rows}
        </table>
        
        <h2>Last 20 Sensor Readings</h2>
        <table>
            <tr><th>Timestamp</th><th>Temperature (°C)</th><th>Pressure (PSI)</th><th>RPM</th></tr>
            {html_rows}
        </table>
    </body>
    </html>
...
def save_settings(n_clicks, refresh, ai, notif, export_fmt, tw, tc, pw, pc, rw, rc, store):
    if not n_clicks:
        raise dash.exceptions.PreventUpdate
    new_store = {
        "refresh":       refresh or 2,
        "ai":            bool(ai),
        "notifications": bool(notif),
...
def render_page_content(pathname, auth_data):
    ...
    if pathname == "/" or pathname == "/operator":
...
def render_process_map():
    ...
    pipe_rows = [
        ...
        ("P-404", "Product Discharge", "2″ CS",  "1.6 m³/h", "55 PSI", "●  Active"),
        ("P-505", "Drain / Vent",      "1″ CS",  "0.2 m³/h", "5 PSI",  "⚬  Standby"),
    ]
    return html.Div([
        html.H3("Process Map"),
        dbc.Row([
            dbc.Col([
                section_label("P&ID Process Flow — Reactor Line 4"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(f"  {s[0]}  ", style={
                                "backgroundColor": "var(--bg-surface)", "color": s[3],
                                "borderRadius": "4px", "padding": "3px 10px",
                                "fontWeight": "700", "fontSize": "10px", "marginRight": "10px",
                                "border": f"1px solid {s[3]}", "letterSpacing": "0.06em",
                            }),
                            html.Span(s[1], style={"fontWeight": "600", "fontSize": "13px", "color": "var(--text-primary)", "marginRight": "8px"}),
                            html.Span(s[2], style={"fontSize": "11px", "color": "var(--text-muted)"}),
                        ], style={"display": "flex", "alignItems": "center", "padding": "10px 0",
                                  "borderBottom": "1px solid var(--border-color)"}),
                        html.Div("  ↓", style={"color": "var(--border-color)", "fontSize": "16px", "padding": "2px 20px"})
                        if i < len(flow_steps) - 1 else html.Div()
                    ])
                    for i, s in enumerate(flow_steps)
                ]), style=cs()),
            ], width=6),
            dbc.Col([
                section_label("Pipeline Register"),
                dbc.Card(dbc.CardBody([
                    html.Table([
                        html.Thead(html.Tr([
                            ...
                            for h in ["Tag", "Description", "Size", "Flow", "Pressure", "Status"]
                        ])),
                        html.Tbody([
                            html.Tr([
                                html.Td(cell, style={"fontSize": "12px", "padding": "7px 10px",
                                                      "color": SUCCESS_COLOR if "Active" in str(cell) else
                                                               WARNING_COLOR if "Standby" in str(cell) else "var(--text-primary)",
                                                      "borderBottom": "1px solid var(--border-color)"})
                                for cell in row
                ...
                section_label("Utility Connections"),
                dbc.Row([
                    dbc.Col(dbc.Card(dbc.CardBody([
                        html.P("Cooling Water", style={"fontSize": "10px", "color": "var(--text-muted)", "marginBottom": "4px", "fontWeight": "600", "letterSpacing": "0.08em", "textTransform": "uppercase"}),
                        html.P("● Inlet 18°C  /  Outlet 28°C", style={"fontSize": "12px", "color": INFO_COLOR, "marginBottom": 0, "fontWeight": "600"}),
                    ]), style=cs(INFO_COLOR)), width=6),
                    dbc.Col(dbc.Card(dbc.CardBody([
                        html.P("Instrument Air", style={"fontSize": "10px", "color": "var(--text-muted)", "marginBottom": "4px", "fontWeight": "600", "letterSpacing": "0.08em", "textTransform": "uppercase"}),
                        html.P("● 6.5 bar  /  Dew pt −40°C", style={"fontSize": "12px", "color": SUCCESS_COLOR, "marginBottom": 0, "fontWeight": "600"}),
                    ]), style=cs(SUCCESS_COLOR)), width=6),
                ]),
            ], width=6),
        ]),
    ])
...
def render_trends():
    """Trends page with working time-range buttons and live stat cards."""
    return html.Div([
        html.H3("Historical Trends"),
        dbc.Row([
            # ── Time range selector (now has IDs) ──────────────────────
            dbc.Col(dbc.Card(dbc.CardBody([
                section_label("Time Range"),
                dbc.ButtonGroup([
                    dbc.Button("1H",  id="trend-btn-1h",  color="primary",           size="sm", className="me-1", n_clicks=0),
                    dbc.Button("6H",  id="trend-btn-6h",  color="outline-secondary", size="sm", className="me-1", n_clicks=0),
                    dbc.Button("24H", id="trend-btn-24h", color="outline-secondary", size="sm", className="me-1", n_clicks=0),
                    dbc.Button("7D",  id="trend-btn-7d",  color="outline-secondary", size="sm", n_clicks=0),
                ], id="trend-btn-group"),
                html.Div(id="trend-range-label", style={
                    "marginTop": "8px", "fontSize": "10px",
                    "color": "var(--text-dimmed)", "letterSpacing": "0.06em"
                }),
            ]), style=cs()), width=4),

            # ── Avg Temperature live ────────────────────────────────────
            dbc.Col(dbc.Card(dbc.CardBody([
                section_label("Avg Temperature (last 10 readings)"),
                html.H4("—", id="avg-temp-display", style={
                    "color": "#FF7043", "fontWeight": "700", "marginBottom": 0,
            ...
            # ── Avg Pressure live ───────────────────────────────────────
            dbc.Col(dbc.Card(dbc.CardBody([
                section_label("Avg Pressure (last 10 readings)"),
                html.H4("—", id="avg-pres-display", style={
                    "color": "#42A5F5", "fontWeight": "700", "marginBottom": 0,
                    "fontSize": "22px", "letterSpacing": "-0.02em"
                }),
            ]), style=cs()), width=4),
        ], className="mb-3"),
        ...
        dbc.Card(dbc.CardBody([
            section_label("Temperature · Pressure · RPM — Live"),
            dcc.Graph(id="full-trend-graph", style={"height": "55vh"}),
        ]), style=cs()),
        html.Div(style={"height": "12px"}),
        section_label("Alarm Threshold Reference"),
        dbc.Card(dbc.CardBody([
            html.Table([
                html.Thead(html.Tr([
                    html.Th(h, style={"color": ACCENT_COLOR, "fontSize": "9px", "fontWeight": "700",
                    ...
                    for h in ["Parameter", "Normal Range", "Warning Threshold", "Critical Threshold", "Unit"]
                ])),
                html.Tbody([
                    html.Tr([html.Td(c, style={"fontSize": "12px", "padding": "8px 14px", "color": "var(--text-primary)", "borderBottom": "1px solid var(--border-color)"}) for c in row])
                    for row in [
                        ("Temperature", "80 – 110",   "> 110", "> 130", "°C"),
                        ("Pressure",    "80 – 150",   "< 80",  "< 60",  "PSI"),
                        ("RPM",         "1000 – 1700", "> 1700","> 2000","RPM"),
                    ]
                ])
            ], style={"width": "100%", "borderCollapse": "collapse"})
        ]), style=cs()),
...
def render_alarms(auth_data=None):
    """Alarms page — stat cards wired to live data, ACK / Suppress buttons working."""
    # Try real alerts, fall back to demo
    real_alerts = fetch_real_alerts(auth_data) if auth_data else []
    return html.Div([
        html.H3("Alarm Management"),
        # ── Stat cards: unack and ack are now dynamic ──────────────────
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
            ...
            dbc.Col(dbc.Card(dbc.CardBody([
                html.P("Acknowledged", style={"fontSize": "9px", "fontWeight": "700", "color": "var(--text-dimmed)",
                                             "textTransform": "uppercase", "letterSpacing": "0.12em", "marginBottom": "8px"}),
                html.H3("0", id="alarm-ack-count", style={"color": WARNING_COLOR, "fontWeight": "700",
                                                          "fontSize": "24px", "letterSpacing": "-0.02em", "marginBottom": "4px"}),
                html.P("Awaiting clearance", style={"fontSize": "11px", "color": "var(--text-muted)", "marginBottom": 0}),
            ]), style=cs(), className="metric-card"), width=3),

            dbc.Col(dbc.Card(dbc.CardBody([
                html.P("Suppressed", style={"fontSize": "9px", "fontWeight": "700", "color": "var(--text-dimmed)",
                                           "textTransform": "uppercase", "letterSpacing": "0.12em", "marginBottom": "8px"}),
                html.H3("0", id="alarm-supp-count", style={"color": INFO_COLOR, "fontWeight": "700",
                                                           "fontSize": "24px", "letterSpacing": "-0.02em", "marginBottom": "4px"}),
                html.P("AI filtered", style={"fontSize": "11px", "color": "var(--text-muted)", "marginBottom": 0}),
            ]), style=cs(), className="metric-card"), width=3),

            dbc.Col(stat_card("Cleared Today",  "14", "Since 00:00",        SUCCESS_COLOR), width=3),
        ], className="mb-3"),

        # ── Action buttons ─────────────────────────────────────────────
        dbc.Row([
            dbc.Col([
                html.Button("✓  ACK ALL ACTIVE",
                    id="alarm-ack-all-btn", n_clicks=0,
                    style={"background": "#FFB300", "border": "none", "borderRadius": "4px",
                ...
                html.Button("⊘  SUPPRESS LAST",
                    id="alarm-supp-btn", n_clicks=0,
                    style={"background": "transparent", "border": "1px solid #26C6DA",
                           "borderRadius": "4px", "color": "#26C6DA", "fontSize": "11px",
                           "fontWeight": "700", "padding": "9px 20px", "cursor": "pointer",
                           "letterSpacing": "0.08em", "textTransform": "uppercase",
                           "fontFamily": "Inter, sans-serif", "transition": "all 0.18s ease"}),
            ], width=12, style={"marginBottom": "16px"}),
        ]),

        dbc.Row([
            dbc.Col([
                section_label("Active Alarms — Prioritized"),
                html.Div(id="full-alarms-container"),
            ], width=8),
            dbc.Col([
                section_label("Alarm Frequency (last shift)"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(label, style={"fontSize": "10px", "color": "var(--text-muted)", "width": "100px", "display": "inline-block", "fontWeight": "600", "letterSpacing": "0.04em"}),
                            html.Div(style={"display": "inline-block", "height": "6px", "width": f"{pct}%", "background": color, "borderRadius": "2px", "verticalAlign": "middle"}),
                            html.Span(f" {count}", style={"fontSize": "11px", "color": "var(--text-primary)", "marginLeft": "6px"}),
                        ], style={"marginBottom": "12px"})
                        for label, pct, count, color in [
                            ("Temperature", 75, "9 events", "#FF7043"),
                            ("Pressure",    45, "5 events", "#42A5F5"),
                            ("RPM",         20, "2 events", "#66BB6A"),
                        ]
                    ]),
                ]), style=cs()),
                html.Div(style={"height": "12px"}),
                section_label("Response Time SLA"),
                dbc.Card(dbc.CardBody([
                    html.Div([html.Span("Critical  → Ack within", style={"fontSize": "11px", "color": "var(--text-muted)"}), html.Span(" 5 min", style={"fontSize": "12px", "color": CRITICAL_COLOR, "fontWeight": "700", "marginLeft": "6px"})], style={"marginBottom": "8px"}),
                    html.Div([html.Span("Warning  → Ack within", style={"fontSize": "11px", "color": "var(--text-muted)"}), html.Span(" 15 min", style={"fontSize": "12px", "color": WARNING_COLOR, "fontWeight": "700", "marginLeft": "6px"})], style={"marginBottom": "8px"}),
                    html.Div([html.Span("Info       → Ack within", style={"fontSize": "11px", "color": "var(--text-muted)"}), html.Span(" 60 min", style={"fontSize": "12px", "color": INFO_COLOR, "fontWeight": "700", "marginLeft": "6px"})]),
                ]), style=cs()),
            ], width=4),
        ]),
    ])
...
def render_events():
    now = datetime.now()
    event_rows = [
        ((now - timedelta(seconds=0)).strftime("%H:%M:%S"),   "System",     "INFO",    "Dashboard initialized — NEXUS IQ v2.1"),
        ((now - timedelta(seconds=14)).strftime("%H:%M:%S"),  "Operator-1", "ACTION",  "Acknowledged alarm: High Temperature R-401"),
        ((now - timedelta(seconds=31)).strftime("%H:%M:%S"),  "AI Engine",  "INSIGHT", "Anomaly pattern detected on Sensor TT-101"),
        ((now - timedelta(seconds=47)).strftime("%H:%M:%S"),  "Scheduler",  "INFO",    "Scheduled maintenance job queued for Pump A"),
        ((now - timedelta(seconds=65)).strftime("%H:%M:%S"),  "System",     "WARNING", "Pressure sensor P-202 reading at lower boundary"),
        ((now - timedelta(seconds=82)).strftime("%H:%M:%S"),  "Operator-2", "ACTION",  "RPM alarm suppressed for 15 minutes"),
        ((now - timedelta(seconds=110)).strftime("%H:%M:%S"), "System",     "INFO",    "Backup data sync completed — 300 records"),
        ((now - timedelta(seconds=134)).strftime("%H:%M:%S"), "AI Engine",  "INSIGHT", "No anomaly: Cooling loop CL-1 nominal"),
    ]
    color_map = {"INFO": "var(--text-primary)", "ACTION": ACCENT_COLOR, "INSIGHT": INFO_COLOR, "WARNING": WARNING_COLOR}
    return html.Div([
        html.H3("System Events"),
        dbc.Row([
            dbc.Col(stat_card("Total Events Today", "148", "Since 00:00 shift start", ACCENT_COLOR),  width=3),
            dbc.Col(stat_card("Operator Actions",   "23",  "Ack / suppress / notes",  SUCCESS_COLOR), width=3),
            dbc.Col(stat_card("AI Engine Events",   "61",  "Insights & predictions",  INFO_COLOR),    width=3),
            dbc.Col(stat_card("System Auto-events", "64",  "Scheduler + sync",        "var(--text-muted)"), width=3),
        ], className="mb-3"),
        section_label("Event Log — Most Recent First"),
        dbc.Card(dbc.CardBody([
            html.Table([
                html.Thead(html.Tr([
                    html.Th(h, style={"color": ACCENT_COLOR, "fontSize": "9px", "fontWeight": "700",
                                       "padding": "6px 14px", "letterSpacing": "0.1em",
                                       "textTransform": "uppercase", "borderBottom": "1px solid var(--border-color)"})
                    for h in ["Time", "Source", "Type", "Description"]
                ])),
                html.Tbody([
                    html.Tr([
                        html.Td(row[0], style={"fontSize": "11px", "padding": "8px 14px", "color": "var(--text-dimmed)", "fontFamily": "monospace", "borderBottom": "1px solid var(--border-color)"}),
                        html.Td(row[1], style={"fontSize": "12px", "padding": "8px 14px", "color": "var(--text-primary)", "fontWeight": "500", "borderBottom": "1px solid var(--border-color)"}),
                        html.Td(
                            html.Span(row[2], style={
                                "fontSize": "9px", "fontWeight": "700", "padding": "2px 7px",
                                "borderRadius": "3px", "letterSpacing": "0.08em", "textTransform": "uppercase",
                                "backgroundColor": "var(--bg-surface)",
                                "color": color_map.get(row[2], "var(--text-primary)"),
                                "border": f"1px solid {color_map.get(row[2], 'var(--border-color)')}",
                            }),
                            style={"padding": "8px 14px", "borderBottom": "1px solid var(--border-color)"}
                        ),
                        html.Td(row[3], style={"fontSize": "12px", "padding": "8px 14px", "color": "var(--text-primary)", "borderBottom": "1px solid var(--border-color)"}),
...
def render_equipment(auth_data):
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col(stat_card("Total Equipment",  "24", "Across all lines",    ACCENT_COLOR),   width=3),
            dbc.Col(dbc.Card(dbc.CardBody([
...
def detect_anomalies(history_deque):
    if not history_deque:
        return []
    
    # Take last 50 readings
    readings = list(history_deque)[-50:]
    if not readings:
        return []
    ...
    for param in ["temperature", "pressure", "rpm"]:
        vals = [d[param] for d in readings]
        n = len(vals)
        mean = sum(vals) / n
        std = (sum((v - mean)**2 for v in vals) / n) ** 0.5
        latest = vals[-1]
        z_score = (latest - mean) / (std + 1e-9)
        slope = (vals[-1] - vals[0]) / n
        ...
        unit = "°C" if param == "temperature" else "PSI" if param == "pressure" else "RPM"
        
        direction = "upward" if slope > 0 else "downward"
        msg = f"{param.capitalize()} is {abs(z_score):.1f}σ {'above' if z_score > 0 else 'below'} recent mean with {direction} trend of {slope:+.1f}{unit}/reading."
...
def render_ai_insights():
    ...
    return html.Div([
        html.H3("AI Insights"),
        dbc.Row([
            dbc.Col(stat_card("Active Insights", "4",   "This shift",        INFO_COLOR),    width=3),
            dbc.Col(stat_card("Predictions",     "1",   "Within 12 hours",   WARNING_COLOR), width=3),
            dbc.Col(stat_card("Anomalies Found", "1",   "Requires attention",CRITICAL_COLOR),width=3),
            dbc.Col(stat_card("Optimisations",   "1",   "Potential savings", ACCENT_COLOR),  width=3),
        ], className="mb-3"),
        html.Div(id="ai-insights-container"),
        section_label("Predictive Failure Analysis"),
        *[
            dbc.Card(dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        html.Span(badge, style={"fontSize": "9px", "fontWeight": "700", "padding": "2px 8px", "borderRadius": "3px",
                                                "backgroundColor": f"rgba({','.join(str(int(color.lstrip('#')[i:i+2],16)) for i in (0,2,4))},0.1)",
                                                "color": color, "border": f"1px solid {color}", "letterSpacing": "0.08em",
                                                "textTransform": "uppercase", "marginBottom": "8px", "display": "inline-block"}),
                        html.P(title, style={"fontWeight": "700", "fontSize": "14px", "color": "var(--text-primary)", "marginBottom": "8px"}),
                        html.P(desc, style={"fontSize": "12px", "color": "var(--text-muted)", "lineHeight": "1.6", "marginBottom": 0}),
                    ], width=10),
                    dbc.Col([
                        html.Div("●", style={"color": color, "fontSize": "24px", "textAlign": "right"}),
                    ], width=2),
                ])
            ]), style={**cs(color), "marginBottom": "12px"})
            for title, badge, color, desc in insights
        ]
    ])


def render_reports(role="Operator"):
    """Reports page with a working Generate Report button and dropdown."""
    reports = [
        ("Daily Ops Summary",       f"{role} Daily", "Generated", SUCCESS_COLOR,  "Today 06:00"),
        ("Alarm Analysis Report",   "Weekly",        "Generated", ACCENT_COLOR,   "Mon 06:00"),
        ("Equipment Health Report", "Monthly",       "Scheduled", WARNING_COLOR,  "01 Jan 06:00"),
        ("Energy Consumption",      "Monthly",       "Generated", INFO_COLOR,     "01 Dec 06:00"),
    ]
    return html.Div([
        html.H3(f"{role} Reports"),
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
                html.P("Reports Generated", style={"fontSize": "9px", "fontWeight": "700", "color": "var(--text-dimmed)",
                                                   "textTransform": "uppercase", "letterSpacing": "0.12em", "marginBottom": "8px"}),
                html.H3("47", id="reports-gen-count", style={"color": SUCCESS_COLOR, "fontWeight": "700", "fontSize": "24px",
                                                             "letterSpacing": "-0.02em", "marginBottom": "4px"}),
                html.P("This month", style={"fontSize": "11px", "color": "var(--text-muted)", "marginBottom": 0}),
            ]), style=cs(), className="metric-card"), width=3),
            dbc.Col(stat_card("Pending Review",    "2",    "Awaiting sign-off",    WARNING_COLOR), width=3),
            dbc.Col(stat_card("Last Generated",    "06:00","Today",                INFO_COLOR),    width=3),
        ], className="mb-3"),

        # ── Generate Report Panel ─────────────────────────────────────
        dbc.Card(dbc.CardBody([
            section_label("Generate New Report"),
            dbc.Row([
                dbc.Col([
                    html.Label("Report Type", style={"fontSize": "10px", "fontWeight": "700",
                                                     "color": "var(--text-muted)", "letterSpacing": "0.1em",
                                                     "textTransform": "uppercase", "marginBottom": "6px", "display": "block"}),
                    dcc.Dropdown(
                        id="report-type-select",
                        options=[
                            {"label": "Daily Ops Summary",       "value": "Daily Ops Summary"},
                            {"label": "Alarm Analysis Report",   "value": "Alarm Analysis Report"},
                            {"label": "Equipment Health Report", "value": "Equipment Health Report"},
                            {"label": "Energy Consumption",      "value": "Energy Consumption"},
                            {"label": "Shift Performance",       "value": "Shift Performance"},
                        ],
                        value="Daily Ops Summary",
                        clearable=False,
                ...
                dbc.Col([
                    ...
                    html.Button("⬡  GENERATE REPORT",
                        ...
                        style={"background": "#00E676", "border": "none", "borderRadius": "4px",
                               ...
                               "transition": "all 0.18s ease", "marginTop": "2px"}),
                ], width=6, style={"display": "flex", "alignItems": "flex-end"}),
            ]),
            html.Div(id="report-gen-feedback", style={"display": "none"}),
        ]), style=cs(ACCENT_COLOR), className="mb-3"),

        section_label("Report Queue"),
        dbc.Card(dbc.CardBody([
            html.Table([
                html.Thead(html.Tr([
                    html.Th(h, style={"color": ACCENT_COLOR, "fontSize": "9px", "fontWeight": "700",
                                       "padding": "7px 14px", "letterSpacing": "0.1em", "textTransform": "uppercase",
                                       "borderBottom": "1px solid var(--border-color)"})
                    for h in ["Report Name", "Type", "Status", "Next Run"]
                ])),
                html.Tbody([
                    html.Tr([
                        html.Td(r[0], style={"fontSize": "12px", "padding": "9px 14px", "fontWeight": "600", "color": "var(--text-primary)", "borderBottom": "1px solid var(--border-color)"}),
                        html.Td(r[1], style={"fontSize": "11px", "padding": "9px 14px", "color": "var(--text-muted)", "borderBottom": "1px solid var(--border-color)"}),
                        html.Td(html.Span(f"● {r[2]}", style={"fontSize": "11px", "fontWeight": "700", "color": r[3]}), style={"padding": "9px 14px", "borderBottom": "1px solid var(--border-color)"}),
                        html.Td(r[4], style={"fontSize": "11px", "padding": "9px 14px", "color": "var(--text-muted)", "fontFamily": "monospace", "borderBottom": "1px solid var(--border-color)"}),
                    ]) for r in reports
                ])
            ], style={"width": "100%", "borderCollapse": "collapse"})
        ]), style=cs()),
...
def render_settings():
    """Settings page with working Save Settings button and interactive controls."""
    return html.Div([
        html.H3("Settings"),
        dbc.Row([
            # ── Left: System Config ──────────────────────────────────
            dbc.Col([
                section_label("System Configuration"),
                dbc.Card(dbc.CardBody([
                    # Refresh Interval
                    html.Div([
                        html.Label("Data Refresh Interval", style={"fontSize": "10px", "fontWeight": "700",
                                                                   "color": "var(--text-muted)", "letterSpacing": "0.1em",
                                                                   "textTransform": "uppercase", "marginBottom": "6px", "display": "block"}),
                        dcc.Dropdown(
                    ...
                    html.Div([
                        ...
                        dcc.Checklist(
                            ...
                            options=[{"label": "  Enable AI Insight Engine", "value": True}],
                            value=[True],
                    ...
                    html.Button("⬡  SAVE SETTINGS",
                        id="settings-save-btn", n_clicks=0,
                        style={"background": "#00E676", "border": "none", "borderRadius": "4px",
                               "color": "#09090b", "fontSize": "11px", "fontWeight": "700",
                               "padding": "10px 24px", "cursor": "pointer", "letterSpacing": "0.08em",
                               "textTransform": "uppercase", "fontFamily": "Inter, sans-serif",
                ...
                dbc.Card(dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            dbc.Input(id="settings-rpm-warn", type="number", style={"backgroundColor": "var(--bg-surface)", "color": "var(--text-primary)", "borderColor": "var(--border-color)", "fontSize": "12px", "marginBottom": "0"})
                        ], width=6),
                        dbc.Col([
                            html.Label("RPM Crit", style={"fontSize": "10px", "fontWeight": "700", "color": "var(--text-muted)", "textTransform": "uppercase", "marginBottom": "4px"}),
                            dbc.Input(id="settings-rpm-crit", type="number", style={"backgroundColor": "var(--bg-surface)", "color": "var(--text-primary)", "borderColor": "var(--border-color)", "fontSize": "12px", "marginBottom": "0"})
                        ], width=6),
                    ]),
                ]), style=cs()),
                html.Div(style={"height": "12px"}),
                ...
                dbc.Card(dbc.CardBody([
                    html.Div([
                        ...
                        for label, status, color in [
                            ...
                            ("Backend API",       "Demo",    WARNING_COLOR),
...
def render_engineer_diagnostics():
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                section_label("Loop Test Results — Last 24h"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(name, style={"fontSize": "11px", "color": "var(--text-primary)", "flex": "1"}),
                            html.Div(style={"display": "inline-block", "height": "6px", "width": f"{pct}%", "background": color, "borderRadius": "2px", "verticalAlign": "middle", "marginRight": "8px"}),
                            html.Span(f"{pct}%", style={"fontSize": "11px", "color": color, "fontWeight": "700"}),
                        ], style={"display": "flex", "alignItems": "center", "marginBottom": "12px"})
                        for name, pct, color in [
                            ("Analog Input Loops",  98, SUCCESS_COLOR),
                            ("Digital I/O Points",  100, SUCCESS_COLOR),
                            ("Control Loops (PID)", 91, WARNING_COLOR),
                            ("HART Devices",        86, WARNING_COLOR),
                        ]
                    ])
                ]), style=cs()),
            ], width=6),
            dbc.Col([
                section_label("Recent Comm Errors"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(dev, style={"fontSize": "11px", "color": "var(--text-primary)", "fontWeight": "600", "flex": "1"}),
                            html.Span(f"{n} errors", style={"fontSize": "11px", "color": WARNING_COLOR}),
                        ], style={"display": "flex", "justifyContent": "space-between", "padding": "7px 0", "borderBottom": "1px solid var(--border-color)"})
                        for dev, n in [("HART Loop 2 — Dev 3", 4), ("Modbus RTU Bus B — Node 14", 3), ("Profibus DP — Node 8", 2), ("Ethernet/IP — PLC-03", 1)]
                    ])
                ]), style=cs()),
            ], width=6),
        ]),
...
def render_engineer_calibration():
    ...
    return html.Div([
        ...
        html.Div(style={"height": "12px"}),
        section_label("Calibration Procedure — Quick Reference"),
        dbc.Card(dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    html.P("Temperature Sensors (TT)", style={"fontSize": "12px", "fontWeight": "700", "color": "#FF7043", "marginBottom": "8px"}),
                    html.P("Use FLUKE 724 Temperature Calibrator. Apply 5-point calibration at 0°C, 50°C, 100°C, 150°C, 200°C. Record as-found and as-left data. Update CMMS.", style={"fontSize": "12px", "color": "var(--text-muted)", "lineHeight": "1.6"}),
                ], width=4),
                dbc.Col([
                    html.P("Pressure Sensors (PT)", style={"fontSize": "12px", "fontWeight": "700", "color": "#42A5F5", "marginBottom": "8px"}),
                    html.P("Use FLUKE 719 Pressure Calibrator. Apply 5-point calibration at 0%, 25%, 50%, 75%, 100% of span. Verify zero and span adjustments.", style={"fontSize": "12px", "color": "var(--text-muted)", "lineHeight": "1.6"}),
                ], width=4),
                dbc.Col([
                    html.P("Flow / Vibration Sensors", style={"fontSize": "12px", "fontWeight": "700", "color": SUCCESS_COLOR, "marginBottom": "8px"}),
                    html.P("Contact CMMS scheduler to book out the loop. Follow procedure ITP-CAL-003 for flow and ITP-CAL-007 for vibration. Requires dual-sign-off.", style={"fontSize": "12px", "color": "var(--text-muted)", "lineHeight": "1.6"}),
                ], width=4),
            ])
        ]), style=cs()),
    ])
...
def render_engineer_signals():
    return html.Div([
        ...
        dbc.Row([
            dbc.Col(
                dcc.Input(id="signal-search", type="text", placeholder="Filter by tag, type, or description...", debounce=True, style={"width": "100%", "backgroundColor": "var(--bg-surface)", "color": "var(--text-primary)", "border": "1px solid var(--border-color)", "borderRadius": "4px", "padding": "8px 12px", "fontSize": "12px"}),
                width=9
            ),
            dbc.Col([
                html.Button("⬇ Export Signal List", id="signal-export-btn", style={"width": "100%", "backgroundColor": "var(--bg-surface)", "color": "#00E676", "border": "1px solid #00E676", "borderRadius": "4px", "padding": "8px 12px", "fontSize": "12px", "fontWeight": "700", "cursor": "pointer"}),
    ...
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                html.Div(style={"height": "12px"}),
                section_label("Shift Roster"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(role, style={"fontSize": "11px", "color": "var(--text-primary)", "flex": "1"}),
                            html.Span(f"{act}/{tot}", style={"fontSize": "12px", "color": SUCCESS_COLOR if act == tot else WARNING_COLOR, "fontWeight": "700"}),
                        ], style={"display": "flex", "justifyContent": "space-between", "padding": "7px 0", "borderBottom": "1px solid var(--border-color)"})
                        for role, act, tot in [("Control Room Ops", 4, 4), ("Field Operators", 8, 8), ("Maintenance", 6, 8), ("Lab / QC", 3, 3), ("Supervision", 3, 3)]
                    ])
                ]), style=cs()),
            ], width=6),
        ]),
...
def render_manager_performance():
    return html.Div([
                ...
                figure=go.Figure(
                    data=[
                        go.Bar(name="Actual", x=["Mon","Tue","Wed","Thu","Fri","Sat","Sun"],
                               y=[148, 155, 140, 162, 145, 138, 142],
                               marker_color=ACCENT_COLOR, opacity=0.85),
                        go.Scatter(name="Target", x=["Mon","Tue","Wed","Thu","Fri","Sat","Sun"],
                                   y=[155,155,155,155,155,155,155],
                                   mode="lines", line=dict(color=WARNING_COLOR, dash="dash", width=2)),
                    ],
                    layout=go.Layout(
        ...
        dbc.Row([
            dbc.Col([
                section_label("Production Line Performance"),
                dbc.Card(dbc.CardBody([
                    html.Table([
                        html.Thead(html.Tr([
                            html.Th(h, style={"color": WARNING_COLOR, "fontSize": "9px", "fontWeight": "700",
                                               "padding": "6px 10px", "letterSpacing": "0.1em",
                                               "textTransform": "uppercase", "borderBottom": "1px solid var(--border-color)"})
                            for h in ["Line", "Output Today", "Target", "OEE", "Status"]
                        ])),
                        html.Tbody([
                            html.Tr([
                                html.Td(row[0], style={"fontSize": "12px", "padding": "8px 10px", "fontWeight": "600", "color": "var(--text-primary)", "borderBottom": "1px solid var(--border-color)"}),
                                html.Td(row[1], style={"fontSize": "12px", "padding": "8px 10px", "color": "var(--text-primary)", "borderBottom": "1px solid var(--border-color)"}),
                                html.Td(row[2], style={"fontSize": "12px", "padding": "8px 10px", "color": "var(--text-muted)", "borderBottom": "1px solid var(--border-color)"}),
                                html.Td(row[3], style={"fontSize": "12px", "padding": "8px 10px", "fontWeight": "700",
                                        "color": SUCCESS_COLOR if float(row[3].rstrip("%")) >= 88 else WARNING_COLOR,
                                        "borderBottom": "1px solid var(--border-color)"}),
                                html.Td(html.Span(f"● {row[4]}", style={"fontSize": "11px", "fontWeight": "700",
                                        "color": SUCCESS_COLOR if row[4] == "Running" else WARNING_COLOR}),
                                    style={"padding": "8px 10px", "borderBottom": "1px solid var(--border-color)"}),
                            ]) for row in [
                                ("Reactor Line 4",  "58 t",  "62 t",  "93.5%", "Running"),
                                ("Reactor Line 3",  "52 t",  "58 t",  "89.7%", "Running"),
                                ("Separator Train", "32 t",  "35 t",  "91.4%", "Running"),
                            ]
                        ])
                    ], style={"width": "100%", "borderCollapse": "collapse"})
                ]), style=cs()),
            ...
            dbc.Col([
                section_label("Energy Mix"),
                dbc.Card(dbc.CardBody([
                    html.Div([
                        html.Div([
                            html.Span(src, style={"fontSize": "11px", "color": "var(--text-muted)", "width": "110px", "display": "inline-block"}),
                            html.Div(style={"display": "inline-block", "height": "6px", "width": f"{pct}%", "background": color, "borderRadius": "2px", "verticalAlign": "middle"}),
                            html.Span(f"  {val}", style={"fontSize": "11px", "color": color, "fontWeight": "700", "marginLeft": "6px"}),
                        ], style={"marginBottom": "12px"})
                        for src, pct, val, color in [
                            ("Grid Power",  70, "108 MWh", INFO_COLOR),
                            ("Solar Gen.",  20, "31 MWh",  SUCCESS_COLOR),
                            ("Gas Turbine", 10, "15 MWh",  WARNING_COLOR),
                        ]
                    ])
                ]), style=cs()),
            ], width=5),
    ...
    return html.Div([
        ...
        dbc.Card(dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dbc.Card(dbc.CardBody([
                        html.P(kpi[1], style={"fontSize": "9px", "fontWeight": "700", "color": "var(--text-dimmed)",
                                              "textTransform": "uppercase", "letterSpacing": "0.12em", "marginBottom": "4px"}),
                        html.P(kpi[0], style={"fontSize": "11px", "color": "var(--text-muted)", "marginBottom": "8px"}),
                        html.H3(kpi[2], style={"color": kpi[5], "fontWeight": "700", "fontSize": "22px", "letterSpacing": "-0.02em", "marginBottom": "4px"}),
                        html.Div([
                            html.Span("Target: ", style={"fontSize": "10px", "color": "var(--text-dimmed)"}),
                            html.Span(kpi[3], style={"fontSize": "10px", "color": "var(--text-muted)", "fontWeight": "600"}),
                            html.Span(f"  {kpi[4]}", style={"fontSize": "10px", "color": kpi[5], "fontWeight": "700", "marginLeft": "8px"}),
                        ]),
                    ]), style={**cs(kpi[5]), "marginBottom": "12px"}, className="metric-card")
                ], width=3) for kpi in kpi_data
            ])
        ]), style={"backgroundColor": "var(--bg-card)", "border": "none", "borderRadius": "8px"}),
        section_label("7-Day OEE Trend"),
        dbc.Card(dbc.CardBody([
            dcc.Graph(
                figure={
                    "data": [
                        {
                            ...
                            "y": [84.2, 87.1, 91.3, 86.8, 88.4, 85.9, 87.4],
                            "type": "scatter",
                            "mode": "lines",
                            "line": {"color": "#00E676"},
                            "fill": "tozeroy",
                    ...
                    "layout": {
                        ...
                        "yaxis": {"showgrid": True, "gridcolor": "rgba(255,255,255,0.05)", "fixedrange": True, "range": [80, 100]},
                        "showlegend": False,
                        "hovermode": "x unified"
                    }
                },
        ...
        ]), style={"backgroundColor": "var(--bg-card)", "border": "none", "borderRadius": "8px", "marginTop": "16px"})
    ])
            ...
            html.Table([
                ...
                html.Tbody([
                    html.Tr([
                        ...
                        html.Td(s[7], style={"fontSize": "12px", "padding": "9px 12px", "color": CRITICAL_COLOR if s[7] != "Normal" else SUCCESS_COLOR, "fontWeight": "600" if s[7] != "Normal" else "400", "borderBottom": "1px solid var(--border-color)"}),
                    ]) for s in shifts
                ])
            ], style={"width": "100%", "borderCollapse": "collapse"})
        ]), style=cs()),
...
def render_manager_compliance():
    ...
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col(stat_card("Next Audit",       "20 Dec","ISO 45001 Surv.",   INFO_COLOR),     width=3),
...
def update_overview(data, settings):
    if not data:
        raise dash.exceptions.PreventUpdate
...
def update_trends(data, time_range):
    ...
    # How many data points to show (2s interval)
    points_map = {"1H": 1800, "6H": 10800, "24H": 43200, "7D": 302400}
    # We cap at MAX_HISTORY since we only store 300 points
    ...
    range_label = label_map.get(time_range, "")

    # Button highlight state
    def btn_color(btn_range):
        return "primary" if btn_range == time_range else "outline-secondary"

    return (fig, avg_t, avg_p, range_label,
            btn_color("1H"), btn_color("6H"), btn_color("24H"), btn_color("7D"))
...
def update_full_alarms(data, ack_store, settings):
    ...
    for i, alert_tuple in enumerate(alerts_raw):
        level = alert_tuple[0]
        msg = alert_tuple[1]
        alert_id = alert_tuple[2] if len(alert_tuple) > 2 else i
        color = {"CRITICAL": "danger", "WARNING": "warning", "INFO": "info"}.get(level, "info")
        alert_divs.append(
            html.Div([
                dbc.Alert(msg, color=color, className="mb-0",
                          style={"marginBottom": "0", "display": "inline-block", "width": "calc(100% - 180px)"}),
                html.Button("✓ ACK", className="ack-btn", id={"type":"alarm-row-ack","index": alert_id}),
                html.Button("⊘ SUPP", className="supp-btn", id={"type":"alarm-row-supp","index": alert_id}),
...
def update_ai_insights(data):
    ...
    for insight in insights:
        ...
        c = color.lstrip('#')
        if len(c) == 6:
            rgba_bg = f"rgba({int(c[0:2],16)}, {int(c[2:4],16)}, {int(c[4:6],16)}, 0.1)"
        else:
            rgba_bg = "rgba(255,255,255,0.1)"
...
def update_report_count(store):
    if store:
        return str(store.get("count", 47))
    return "47"


# ── SETTINGS PRE-POPULATE FROM STORE ──────────────────────────────────────────
@app.callback(
...
def update_interval(store):
    store = store or {}
    return store.get("refresh", 2) * 1000
...
def update_signals_table(search_query):
    ...
    if search_query:
        ...
        for s in SIGNALS_DATA:
            if sq in s[0].lower() or sq in s[2].lower() or sq in s[4].lower():
                filtered_signals.append(s)
    else:
        filtered_signals = SIGNALS_DATA

    ok_count = sum(1 for s in filtered_signals if s[6] == "OK")
    drift_count = sum(1 for s in filtered_signals if s[6] == "Drift")
    fault_count = sum(1 for s in filtered_signals if s[6] == "Fault")
...
# ── ALARM STATS POLLING (backend-driven counts) ─────────────────────────────
@app.callback(
...
def poll_alarm_stats(n, auth_data):
    ...
    try:
        r = requests.get(
            "http://localhost:8000/api/data/alerts/stats",
            headers={"Authorization": f"Bearer {auth_data['token']}"},
            timeout=2
        )
        if r.status_code == 200:
            ...
            unack_str = "0 ACTIVE" if unack == 0 else f"{unack} ACTIVE"
            return unack_str, str(stats.get("acknowledged", 0)), str(stats.get("suppressed", 0))
    except Exception:
        pass
    raise dash.exceptions.PreventUpdate
